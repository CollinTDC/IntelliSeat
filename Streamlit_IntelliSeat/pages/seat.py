...
def add_seat(building_id, seat_type):
    ...
    new_seat_data = pd.DataFrame({
        "building_id": [building_id],
        "seat_id": [seat_id],
        "seat_type": [seat_type]
    })

    df_seats = pd.concat([df_seats, new_seat_data], ignore_index=True)
...
def get_seat_type(seat_id):
    df_seats = load_seat_dataframe
    return df_seats[df_seats["seat_id"] == seat_id]["seat_type"].values[0]

def get_seat_types_for_all_seats(seat_ids):
    seat_types = []
    
    for seat_id in seat_ids:
        seat_type = get_seat_type(seat_id)
