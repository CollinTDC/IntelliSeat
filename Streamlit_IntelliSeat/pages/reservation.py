...
def get_seats_in_building(building_id, date):
...
def get_free_seats_in_building_on_day(building_id, date_str):
    ...
    seats_in_building = get_seats_in_building(building_id, date_str)
    ...
    for seat in seats_in_building:
        reservations_for_seat_on_date = df_reservations.loc[(df_reservations['seat_id'] == seat) & (df_reservations['date'] == date_str)]
    ...
    for seat in reserved_seats_building:
        ...
        date = df_reservations[df_reservations['seat_id'] == seat, 'date'].values[0]
...
def get_amount_of_free_seats(building_id, date_str):
    ...
    date = datetime.strptime(date_str, '%Y-%m-%d').date()
    empty_seats = get_free_seats_in_building_on_day(building_id, date_str)

    return f"On {date_str}, there are {len(empty_seats)} spaces left in {building_id}"
